...
const CLEAR_SERVER_LINE =
    '        server %s %s:81 check inter 30s slowstart 10s\n';
const SSL_SERVER_LINE =
    '        server %s %s:80 check inter 30s slowstart 10s\n';
...
function writeHaproxyConfig(opts, cb) {
    ...
    for (var name in Object.keys(opts.servers)) {
            clear += sprintf(CLEAR_SERVER_LINE, name,
...
function reload(opts, cb) {
    ...
    if (RELOAD_RUNNING) {
        ...
        opts.log.debug('Servers we are saving for queued reload: %s',
            opts.servers);
    ...
    vasync.pipeline({ arg: tmpOpts, funcs: [
        ...
        if (RELOAD_NEEDS_RUN) {
            ...
            opts.log.debug('Calling queued reload, using saved servers: %s',
                          RELOAD_OPTS.servers);
