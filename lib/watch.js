 ...
 */
ServerWatcherFSM._processRemovals = function (servers) {
    var self = this;
    var log = this.sw_log;
    ...
    if (removed.length > rmThresh) {
        ...
        toRestore.forEach(function (s) {
            servers.push(self.sw_lastServers[s]);
        });
    ...
    while (removed.length > 0) {
        ...
        log.info('keeping removed server %s around for HOLD_TIME (%d s)',
            sname, HOLD_TIME / 1000);
        servers.push(self.sw_lastServers[sname]);
